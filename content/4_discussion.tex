...
Our experiments further confirm that both the quadratic cost function and the budget constraint are required for QS to perform well. Removing either component (as in UQS or LS) results in diminished effectiveness discussed in~\Cref{sec:result_2}. Pragmatically, rather than adopting LS to reduce cognitive load, future research should focus on developing interfaces that facilitate QS completion. Scientifically, these results prompt further investigation into the psychological mechanisms underlying the lower performance of LS.
...
Since QS credits and votes are inter-related, we believe that this correction in distortion helped QS votes to reflect reasonably well the presented preferences of the individual, albiet the credits performs slightly better without statistical differences as shown in Figure X.
...
% While donation behavior serves as a proxy for "true preferences" due to its incentive-compatible nature, not all preferences are expressed monetarily. Factors such as previous charitable giving, personal connections, or tax implications may influence donation decisions independently of underlying preferences.
